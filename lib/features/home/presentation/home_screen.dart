...
class _HomeScreenState extends State<HomeScreen> {
              ...
              children: [
                ...
                Positioned(
                  ...
                  child: FloatingActionButton(
                    ...
                    onPressed: () {
                      if (tripController.currentPosition != null) _mapController.move(tripController.currentPosition!, 15.0);
                    },
  ...
  Widget _buildMap(TripController tripController) {
    if (tripController.isLocationLoading) return const Center(child: CircularProgressIndicator());
    ...
    return FlutterMap(
      ...
      children: [
        ...
        if (tripController.currentPosition != null) MarkerLayer(markers: [Marker(width: 80.0, height: 80.0, point: tripController.currentPosition!, child: Icon(Icons.person_pin_circle, color: Colors.teal[700], size: 60.0))])
      ],
    );
  }
  Widget _buildStartTripButton(BuildContext context, LatLng? currentPosition) {
    return Positioned(bottom: 40, left: 24, right: 24, child: ElevatedButton.icon(icon: const Icon(Icons.navigation_outlined), label: const Text('شروع سفر جدید'), style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 20),textStyle: const TextStyle(fontSize: 18)), onPressed: currentPosition == null ? null : () => context.go('/start-trip', extra: currentPosition)));
  }
  Widget _buildInTripControls(BuildContext context) {
    return Positioned(bottom: 40, left: 24, right: 24, child: Row(children: [
      Expanded(child: ElevatedButton.icon(icon: const Icon(Icons.check_circle_outline), label: const Text('اتمام سفر'), style: ElevatedButton.styleFrom(backgroundColor: Colors.green[700], padding: const EdgeInsets.symmetric(vertical: 20), textStyle: const TextStyle(fontSize: 16)), onPressed: () async { await context.read<TripController>().completeActiveTrip(); if(mounted) {ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('سفر با موفقیت به اتمام رسید.')));}})),
      const SizedBox(width: 16),
      Expanded(child: ElevatedButton.icon(icon: const Icon(Icons.sos_outlined), label: const Text('SOS'), style: ElevatedButton.styleFrom(backgroundColor: Colors.red[700], padding: const EdgeInsets.symmetric(vertical: 20), textStyle: const TextStyle(fontSize: 16)), onPressed: () { context.read<TripController>().triggerSOS(); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('پیام اضطراری برای نگهبانان ارسال شد!')));})),
    ]));
  }
}
