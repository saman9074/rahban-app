  ...
  AppRouter(this.authController) {
    router = GoRouter(
      ...
      routes: [
        ...
        // مسیر ocr-scanner حذف شد
      ],
      redirect: (context, state) {
        final currentAuthState = authController.authState;
        final isAuthRoute = state.matchedLocation == '/login' || state.matchedLocation == '/register';
        if (currentAuthState == AuthState.unknown) return null;
        if (currentAuthState == AuthState.authenticated) return isAuthRoute ? '/home' : null;
        else return isAuthRoute ? null : '/login';
      },
    );
  }
}